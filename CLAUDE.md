...
**`subdomain` vs `website`:** `subdomain` is unique and required (the portal host). The onboarding "Website" is the company's site — stored in the non-unique, informational `website` column. (Custom domains are **not** a feature; the `tenants.custom_domain` column is unused/legacy and never written.)
